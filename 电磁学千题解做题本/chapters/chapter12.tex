...
\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_1_1.jpg}
\end{center}
\textit{图 12.1.1(1)}

\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_1_2.jpg}
\end{center}
\textit{图12.1.1(1)}

\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_1_3.jpg}
\end{center}
\textit{图12.1.1(2)}
...
\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_2_1.jpg}
\end{center}
\textit{图12.1.2(1)}

\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_2_2.jpg}
\end{center}
\textit{图12.1.2(2)}
...
\textbf{12.1.4.} 一空气平行板电容器的两极板都是半径为 $5.0\mathrm{cm}$ 的圆形导体片，在充电时，其中电场强度的变化率为 $\frac{\mathrm{d}E}{\mathrm{d}t} = 1.0\times 10^{12}\mathrm{V / (m\cdot s)}$ .略去边缘效应，试求：(1）两极板间的位移电流 $I_{D}$ ；(2）两极板边缘处磁感强度 $\vec{B}$ 的值.
...
\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_5.jpg}
\end{center}
\textit{图12.1.5}
...
\textbf{12.1.8.} 一圆柱形无穷长直导线，半径为 $a$ ，载有稳恒直流 $I$ ， $I$ 均匀地分布在横截面上.试求这导线内外磁场强度 $\vec{H}$ 的旋度 $\nabla \times \vec{H}$

\vfill

\newpage
\textbf{12.1.9.} 在一圆柱形空间内，有均匀的、但是随时间 $t$ 变化的磁场，其磁感强度为 $\vec{B} = B(t)\vec{k}$ ， $\vec{k}$ 是沿圆柱轴线方向上的单位矢量。取笛卡儿坐标系如图12.1.9(1)所示，试证明：在此柱体内离轴线为 $r = \sqrt{x^2 + y^2}$ 处，电场强度为 $\vec{E} = \frac{1}{2} (y\vec{i} - x\vec{j})\frac{\mathrm{d}B}{\mathrm{d}t}$ .

\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_9_1.jpg}
\end{center}
\textit{图12.1.9(1)}

\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_9_2.jpg}
\end{center}
\textit{图12.1.9(2)}
...
\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_10.jpg}
\end{center}
\textit{图12.1.10}
...
\newpage
\textbf{12.1.11.} 一很长的直圆筒，半径为 $R$ ，表面上带有一层均匀电荷，电荷量的面密度为 $\sigma$ .在外力矩的作用下，从 $t = 0$ 时刻开始，以匀角加速度 $\alpha$ 绕它的几何轴转动，如图12.1.11所示.(1)试求筒内的磁感强度 $\vec{B}$ ;(2)试求筒内接近内表面处的电场强度 $\vec{E}$ 和坡印亭矢量 $\vec{S}$ ;(3)试证明：进入这圆筒长为 $l$ 一段的 $\vec{S}$ 的通量等于 $\frac{\mathrm{d}}{\mathrm{d}t}\left(\frac{\pi R^2l}{2\mu_0} B^2\right)$

\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_11.jpg}
\end{center}
\textit{图12.1.11}
...
$$
\oiint_ {S} \vec{B} \cdot \mathrm{d} \vec{S} = 0
$$
...
$$
\oint_ {L} \vec{E} \cdot \mathrm{d} \vec{l} = - \frac {\mathrm{d} \Phi}{\mathrm{d} t}
$$

和安培环路定理

$$
\oint_ {L} \vec{H} \cdot \mathrm{d} \vec{l} = I + \frac {\mathrm{d} \Phi_ {D}}{\mathrm{d} t}
$$

对于 $L_{1} + L^{\prime}$ 和 $L_{2} + L^{\prime}$ 都成立，则它们对于 $L$ 也必定成立.

\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_13_1.jpg}
\end{center}
\textit{图12.1.13}

\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_13_2.jpg}
\end{center}
...
\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_16_1.jpg}
\end{center}
\textit{图12.1.16(1)}

\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_16_2.jpg}
\end{center}
\textit{图 12.1.16(2)}
...
\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_17.jpg}
\end{center}
\textit{图12.1.17}
...
\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_1_20.jpg}
\end{center}
\textit{图12.1.20}
...
\newpage
\textbf{12.2.2.} 已知真空中一平面电磁波的电场强度为 $E = E_0\mathrm{e}^{\mathrm{i}(k\cdot r - \omega t)}$ ，式中 $E_0$ 是常矢量， $\omega$ 是圆频率， $\vec{k}$ 是传播矢量， $\vec{r}$ 是位置矢量， $i = \sqrt{-1}$ ，e是自然对数的底．试由麦克斯韦方程求这电磁波的磁场强度 $\vec{H}$
...
\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_2_8.jpg}
\end{center}
\textit{图12.2.8}
...
\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_2_12_1.jpg}
\end{center}
\textit{图12.2.12}

\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_2_12_2.jpg}
\end{center}
\textit{图 12.2.12(2)}
...
\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_2_13.jpg}
\end{center}
\textit{图12.2.13}
...
\newpage
\textbf{12.2.14.} 半径为 $a$ 的长直导线载有电流 $I, I$ 沿轴线方向并均匀分布在横截面上. 试证明: (1) 在导线表面上, 坡印亭矢量 $\vec{S}$ 的方向处处垂直于表面并向内; (2) 导线体内消耗的焦耳热等于 $\vec{S}$ 输送来的能量.

\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_2_14.jpg}
\end{center}
\textit{图12.2.14}

\vfill

\textbf{12.2.15.} 一同轴电缆由两个共轴的长直导体圆筒构成, 它们的半径分别为 $a$ 和 $b$ , 如图 12.2.15 所示. 当这电缆的一端接上负载 $R$ , 另一端加上电压 $U$ 时, 电流在两筒上都是均匀分布的. (1) 试求电缆内的电场强度 $\vec{E}$ 和磁场强度 $\vec{H}$ 以及坡印亭矢量 $\vec{S}$ ; (2) 试论证, 不论电源的正极是接在内筒上还是接在外筒上, $\vec{S}$ 的方向总是由电源端指向负载端; (3) 试证明, 穿过两筒间横截面的功率等于 $U^2 / R$ , 并说明所得结果的物理意义.


\begin{center}
    \includegraphics[width=0.3\linewidth,height=0.6\textheight,keepaspectratio]{figures/fig_12_2_15.jpg}
\end{center}
\textit{图12.2.15}
